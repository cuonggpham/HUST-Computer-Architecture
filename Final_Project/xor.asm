#Prj Cuoi Ki: Bai 5 - PhamQuocCuong_20225604
.data
	...
	requestMsg: .asciiz "Nhap bieu thuc trung to vao day\n(So tu nhien 00->99, cac toan tu +, -, *, /, (,) )"
	...
	MangHauTo: .space 256
	operator: .space 256
	#converter: .word 1 # Khoi tao bien int: converter
	#wordToConvert: .word 1
	stack: .space 256 
.text
# ___________________________MucLuc_____________________
...
# __________________________Let's Begin!________________
# _______________Thuat toan chuyen Trungto->Hauto_______
	# Xet c = str.at(i)
	#  c là 1 so -> cho vào mang MangHauTo.
	#  c == ( -> ??y vào ng?n x?p Operator<stack>.
	#  c ==  ) , thì lay cac toan tu trong Operator<stack> ra va cho vao output cho den khi g?p "(". Dua "(" ra khoi Operator<stack>
	#  c là l toán t?:
		# neu stack là rong hoac do uu tien c > toan tu top cua Operator<stack> thi dua toan tu hien tai vao Operator.
		# con neu (c=<top of Operator<stack>), lay top cua Operator<stack> cho vao MangHauTo. Tiep tuc so sanh c voi cac top cua Operator cho ?en khi thoa man dieu kien dau tien.
		# Sau khi duyet xong bieu thuc Trung to, ta check xem trong Operator<stack> con phan tu nao khong. N?u c n, lay cac toan tu trong Operator<stack> ra va cho vao MangHauTo.
#_____________________KetThucThuatToan___________________
main:
	jal getMangTrungTo   # D?c bi?u th?c trung t?
	nop
	jal inMangTrungTo  # In ra bi?u th?c nh?p vào
	nop
	jal createVar  # Kh?i t?o bi?n
	nop
	jal scanMangTrungTo # Duy?t bi?u th?c
	nop
...
# BeginPart1___________Doc&Duyet bieu thuc trung to_________
...
inMangTrungTo:
	li $v0, 4 
	la $a0, phepTinhMoi # L?u ??a ch? phepTinhMoi vao $a0 
	syscall 
	li $v0, 4 
...
# Khoi t?o các bi?n và thanh ghi
createVar:
	...
	li $t5, -1 # Kich thuoc hien tai cua MangHauTo
		# Ban dau mang rong, nen kich thuoc la -1
	li $t6, -1 # Kích thuoc Operator
		# Ban dau mang rong, nen kich thuoc la -1
...
endnumberToPost:
	jr $ra
# EndPart1._Ket thuc duyet bieu thuc Trung To_

# Begin Part2_Duyet operatorStack ______
# Doc duoc +, -
toanTuShift:
    # Neu truoc toan tu la toan tu hoac '(' hoac rong -> loi
    ...
    continueToanTuAnd:
        ...
        beq $t7, '+', inputToOp # Neu top dang la '+', luu vao
        beq $t7, '-', inputToOp # Neu top dang la '-', luu vao
        beq $t7, '*', inputToOp # Neu top dang la '*', luu vao
        beq $t7, '/', inputToOp # Neu top dang la '/', luu vao
...
matchBracket:
	...
	j scan
# EndPart2.____________________KetThucScanOperatorStack__________

# BeginPart3____________________InMangHauTo_________________
...
finishPrint:
	...
	jr $t0
	nop
# EndPart3__________KetThucIn___________

# BeginPart4_________TinhGiaTri_________
# ______ThuatToan________
	...
	# $t9 la buoc nhay cua float 4, mang stack
	# $t3 luu dia chi cua stack, dung de tinh bieu thuc hau to
	# $t6 la bien dem cua mang hau to
...
printResult:
	...
	jr $t0
	nop
# EndPart4______________KetThucTinhGiaTri__________________
